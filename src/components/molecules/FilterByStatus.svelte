...
<Button color="alternative" class="bg-gray-100 px-3 py-[0.325rem] md:px-5">
...
<Dropdown bind:open={isDropdownOpen} class="w-[200px]">
	{#each filterOptions as { name, value }}
		<DropdownItem
			class={`${$page.url.searchParams.get('status') === value ? 'text-blue-400' : ''}`}
			onclick={() => handleFilter(value)}>{name}</DropdownItem
		>
	{/each}
</Dropdown>
